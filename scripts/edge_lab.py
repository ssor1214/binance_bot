...
def signals(P, I, FRM=None, M=None):
    ...
    # 라이브 e5_f1_84_95 가 4건에 +0.73(승률 50%)으로 유일한 플러스였다.
    # 4건은 표준편차 0.72% 에서 오차범위 ±0.36% 라 판정 불가다. 같은 게이트를
    # 60일 데이터로 크게 재서 그 플러스가 실체인지 확인한다.
    # 범위 셋을 **함께** 보고한다 — 하나만 보면 사후 선택이 된다.
    for lo, hi in ((0.84, 0.95), (0.80, 1.00), (0.92, 0.94)):
        g = (f1q >= lo) & (f1q <= hi)
        put(f"[F1 {int(lo*100)}~{int(hi*100)}] + CM추세", g & cm_l, g & cm_s)

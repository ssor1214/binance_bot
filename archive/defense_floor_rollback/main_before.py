...
def should_allow_aggressive_limit_fallback(candidate: dict, cfg: Config) -> bool:
    ...
    score = float(candidate.get("score", 0.0))
    min_probability = max(
        float(getattr(cfg, "high_confidence_probability", 1.0)),
        float(getattr(cfg, "low_balance_recovery_min_probability", 1.0)),
    )
    if probability < min_probability:
        return False
    if score < float(getattr(cfg, "low_balance_recovery_min_score", 1.0)):
        return False
    signal = str(candidate.get("signal", "")).upper()
...
def _is_effectively_dust_entry(actual_margin_usdt: float, target_min_margin_usdt: float) -> bool:
    """부분체결이 최소 목표 증거금의 절반도 안 되면 '먼지 포지션'으로 보고 정리 대상."""
    dust_threshold_usdt = max(0.25, target_min_margin_usdt * 0.5)
    return actual_margin_usdt + 1e-9 < dust_threshold_usdt
        ...
        for symbol, pos in list(pm.positions.items()):
            ...
            if pos.stop_loss_widened and cfg.stop_loss_grace_sec > 0:
                ...
                if needs_retighten:
                    ...
                    if _would_stop_market_immediately_trigger(pos.side, stop_price, mark_price):
                        log.warning(
                            "[%s] 손절 재타이트 생략 — triggerPrice=%.8f, markPrice=%.8f 로 즉시발동 위험(-2021 방지), 기존 주문 유지",
                            symbol, stop_price, mark_price,
                        )
                        continue
                    old_stop_order_id = pos.stop_order_id
                    try:
    ...
    if action != "STOP_LOSS" and _deferred_pos is not None and _deferred_pos.sl_defer_until > 0:
        ...
        try:
            ...
            if _deferred_pos.side == "LONG":
                ...
                restore_price = _deferred_pos.entry_price * (1 + restore_pct / 100 / leverage)
            if _would_stop_market_immediately_trigger(_deferred_pos.side, restore_price, mark_price):
                log.warning(
                    "[%s] 손절 유예 후 복원 생략 — triggerPrice=%.8f, markPrice=%.8f 로 즉시발동 위험(-2021 방지), widened stop 유지",
                    symbol, restore_price, mark_price,
                )
                return
            old_id = _deferred_pos.stop_order_id
            restored = ex.place_stop_market(symbol, _deferred_pos.side, _deferred_pos.quantity, restore_price)
...
def scan_entry_candidate(ex: Exchange, cfg: Config, symbol: str, total_balance: float) -> dict | None:
    ...
    if signal is None:
        reject("signal_missing", min_confirmations=int(min_confirmations))
        return None
...
def place_entry_order(ex: Exchange, cfg: Config, symbol: str, side: str, quantity: float,
    ...
    if not aggressive and fallback_allowed and cfg.limit_entry_aggressive_fallback_enabled:
        try:
            ...
            if retry_spread_pct <= cfg.limit_entry_aggressive_max_spread_pct and chase_pct <= cfg.limit_entry_aggressive_max_chase_pct:
                ...
                retry_order = ex.open_limit_position(symbol, side, quantity, retry_price)
    ...
    if tg is not None:
        tg.notify_entry_skipped(
            symbol,
            "지정가 미체결",
            f"{cfg.limit_entry_wait_sec:.0f}초 내 체결이 없어 진입 포기",
        )
    return False
...
def execute_entry(ex: Exchange, pm: PositionManager, cfg: Config, tg: TelegramNotifier, total_balance: float, candidate: dict) -> bool:
    ...
    if base_ratio_before_defense > 0 and cfg.defense_stack_min_ratio_mult > 0:
    ...
    try:
        ...
        if not place_entry_order(
            ...
            signal,
            quantity,
            aggressive=bool(candidate.get("early_entry_spike")) and cfg.spike_entry_aggressive_fill,
    ...
    if pos:
        ...
        if _is_effectively_dust_entry(actual_margin_usdt, target_min_margin_usdt):
            dust_threshold_usdt = max(0.25, target_min_margin_usdt * 0.5)
            log.warning(
            ...
            if total == 0 or agree / total < cfg.mtf_min_agree_ratio:
                if should_allow_zero_agree_mtf_exception(candidate, agree, total, cfg):
                    ...
                    tg.notify_entry_skipped(
                        symbol,
                        "상위 시간대 추세 불일치",
                        f"정합 {agree}/{total}, 최소비율 {cfg.mtf_min_agree_ratio:.2f}",
                        cooldown_sec=180,
                    )
                    continue
